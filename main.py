...
class CafeForm(FlaskForm):
    ...
    location = StringField('Cafe Location on Google Maps', validators=[URL(), DataRequired()])
    ...
    power = SelectField('Access to Power Sockets', choices=['🔌','🔌🔌','🔌🔌🔌','🔌🔌🔌🔌','🔌🔌🔌🔌🔌'], validators=[DataRequired()])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        ...
        d = form.open.data
        opn = d.strftime("%I:%M%p")
        d = form.close.data
        close = d.strftime("%I:%M%p")
        coffee = form.coffee.data
        wifi = form.wifi.data
        power = form.power.data
        new_entry = f"\n{cafe}, {location}, {opn}, {close}, {coffee}, {wifi}, {power}"
